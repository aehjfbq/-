...
bool authenticate(const string& username, const string& password) {
    ...
    // 构建查询语句，检查用户名是否存在
    std::string query = "SELECT password FROM users WHERE username = '" + username + "' LIMIT 1";
...
void parse_post_data(string postData, redis::Redis& redis_client) {
    // 解析 JSON 数据
    Json::Reader reader;
    Json::Value root;
...
int main() {
    ...
    while (FCGI_Accept() >= 0) {
        ...
        parse_post_data(resdata, redis_client);  // 处理 POST 数据
        // 输出 JSON 响应
